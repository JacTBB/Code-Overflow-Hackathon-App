...
export default function Steps() {
  ...
  (async () => {
    ...
    for (var DaysRetrieved = 1; DaysRetrieved <= 10; DaysRetrieved++) {
      ...
      const DayStepCount = (await AsyncStorage.getItem(`StepCount-${CurrentDate.getFullYear()}-${Month}-${Day}`)) || 0
      // @ts-ignore
      StepHistoryStringData.push([`${CurrentDate.getFullYear()}-${Month}-${Day}`, DayStepCount])
    }
  ...
  const StepHistoryItems = ({Item}) => (
    <View style={{flex: 1, flexDirection: 'row'}}>
      <Text style={{ color: 'white', width: 140 }}>{(new Date(Item[0])).toDateString()}</Text>
      <Text style={{ color: 'white', width: 120 }}>{Item[1]}/{StepGoal} Steps</Text>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ flex: 2 }}>
        <Text style={{color: 'white', fontSize: 20, fontWeight: 'bold'}}>Past 10 days step history</Text>
        <FlatList
          data={StepHistory}
